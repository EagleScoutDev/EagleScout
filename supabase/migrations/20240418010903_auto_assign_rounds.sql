...
for pos in 1..6 loop
  ...
  for match in currentMatchStart..stopIdx loop
    INSERT INTO scout_assignments_position_based (competition_id, user_id, match_number, robot_position) VALUES (competition_id_arg, user_ids[currentUserIdx], match, posEnum) ON CONFLICT (competition_id, match_number, robot_position) DO UPDATE SET user_id=user_ids[currentUserIdx];
  end loop;
