...
CREATE OR REPLACE FUNCTION public.add_scout_report(competition_id_arg integer, match_number_arg integer, team_arg integer, form_id_arg integer, data_arg jsonb)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
declare
...
begin
  SELECT id INTO match_id_var
    ...
    insert into scout_reports (match_id, user_id, data, team)
    values (match_id_var, auth.uid(), data_arg, team_arg);
end;
$function$
;
