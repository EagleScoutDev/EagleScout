import { padLeft } from "@/lib/util/padLeft";
    ...
    public static hex(hex: string, fg?: Color) {
        return new Color(
            ...((hex + "FF")
                .match(/[a-z0-9]{2}/gi)!
                .slice(0, 4)
                .map((x) => parseInt(x, 16)) as [number, number, number, number]),
            fg,
        );
    }
    public static parse(str: string) {
        if (str.startsWith("rgb(")) {
            return Color.rgb(...(str.match(/\d+/g)!.slice(0, 3).map(Number) as [number, number, number]));
