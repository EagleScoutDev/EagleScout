...
export function AddCompetitionModal({ visible, setVisible, onRefresh }: AddCompetitionModalProps) {
    ...
    const [selectedPitScoutingFormID, setSelectedPitScoutingFormID] = useState(null);
    ...
    const getFormIDs = async () => {
        // get form ids
        let { data: forms, error } = await supabase.from("forms").select("id, name, pitScouting:pit_scouting");
        if (error) {
    ...
    const submitCompetition = async () => {
        ...
        if (selectedPitScoutingFormID == null) {
            Alert.alert("Error", "Please select a pit scouting form to use for this competition.");
            return false;
        }
        ...
        if (fetchTbaEventError) {
            if (fetchTbaEventError instanceof FunctionsHttpError) {
                ...
                Alert.alert("Error", "An unknown error has occurred. Try again or contact support.");
            }
            return false;
        }
    ...
    return (
        <StandardModal title={"Add Competition"} visible={visible}>
            ...
            <View style={styles.button_row}>
                <StandardButton
                    textColor={getIdealTextColor(parseColor(colors.notification))}
