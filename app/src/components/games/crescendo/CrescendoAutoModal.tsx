...
const CrescendoAutoModal = forwardRef(
  (
    {
      isActive,
      setIsActive,
      ...
      onLabelChange: (label: string, change: number) => void;
    },
    ref,
  ) => {
      ...
      return (
        <Pressable
          ...
          onPress={() => {
            ...
            onLabelChange(CrescendoActions[action].link_name, 1);
          }}>
                  ...
                  if (lastAction) {
                    switch (lastAction.action) {
                      case 'intake':
                        setAutoPath(paths =>
                          paths.filter(
                            path => path.noteId !== lastAction.noteId,
                          ),
                        );
                        break;
                      case 'miss':
                      ...
                      case CrescendoActions[CrescendoActionType.ScoreSpeaker]
                        ...
                        setActionCount(prevActionCount => {
                          let newActionCount = new Map(prevActionCount);
                          newActionCount.set(
                            CrescendoActions[lastAction.noteId]
                              .action as CrescendoActionType,
                            (newActionCount.get(
                              CrescendoActions[lastAction.noteId]
                                .action as CrescendoActionType,
                            ) ?? 0) - 1,
